...
<tr> <td> Sensor_Type</td> <td>  Plan </td> <td> User_name </td> <td> state  <td/></tr>
<?php
	// connect to the database
	$db = mysqli_connect('localhost', 'newuser', 'password', 'sensor_cloud');
	//$un = $_SESSION['username'];
	$query = "select *  from User_requirements";
	...
	if (mysqli_num_rows($result) > 0) 
	{
		while($final_result = mysqli_fetch_assoc($result))  //fetch line by line
		{
			...
			$plan_temp = $final_result['Plan'];
			$un_temp = $final_result['User_name'];
			$state_temp = $final_result['state'];
			echo "<tr> <td>$st_temp </td> <td> $plan_temp </td> <td>$un_temp </td> <td> $state_temp <td/></tr>";
		}
	}
