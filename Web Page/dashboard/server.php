...
if (isset($_POST['reg_user'])) {
  ...
  if (count($errors) == 0) {
  	...
  	$query2 = "insert into Price (username, total_price) values ('$username', 0)";
...
if (isset($_POST['add_requirement'])) {
  ...
  if (empty($Plan)) {
  	array_push($errors, "Plan is required");
  }

  if (count($errors) == 0) {
  	$username = $_SESSION['username'];
  	$query = "insert into User_requirements(Sensor_Type, Plan, User_name) values('$Sensor_Type', '$Plan','$username')";
  	mysqli_query($db, $query);
  	header('location: dashboard.php');
  }
}
